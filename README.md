...
When the same visual error appears in two consecutive revisions or QA exhausts artwork edits, the worker rewrites the creative brief and tries another artwork version without repeating research or selection. `MERCH_MAX_BRIEF_REWRITES` defaults to eight; each version still uses up to `MERCH_MAX_REVISION_ATTEMPTS` artwork edits. The run records every version and its estimated model cost. If all eight rewrites fail, it pauses at `awaiting_brief_revision` with the failed QA artifacts. Edit the brief JSON on the run page or pass `--brief-file revised-brief.json` to `retry-artwork`; an unchanged brief is rejected. Provider failures can still be retried with `docker compose exec -T web .venv/bin/merch retry-artwork <run-id>` after the provider issue is fixed.
