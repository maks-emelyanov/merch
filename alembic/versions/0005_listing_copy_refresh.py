...
def upgrade() -> None:
    op.add_column("workflow_runs", sa.Column("listing_generation_state", sa.JSON(), nullable=True))
    op.create_table(
    ...
    op.create_table(
        "copy_refresh_items",
        sa.Column("id", sa.String(36), primary_key=True),
        sa.Column("batch_id", sa.String(36), sa.ForeignKey("copy_refresh_batches.id", ondelete="CASCADE"), nullable=False),
